# One-Stop MCP: Product Design & Requirements (PDR v0.1)

> This is the original design brief for One-Stop MCP, written in August 2026 as a draft for RFC. It describes the target design, not what is built; for current status see the [README](../README.md#status). It is converted to Markdown from [PDR.html](PDR.html), the styled original, with the content unchanged.
...
| **Least-privilege injection** | keys enter outbound calls only — never context, never logs |
